...
do_check() {
  ...
  if [ -f gen/peg_yaml.sh ]; then
    echo "  ✓ gen/peg_yaml.sh ($(wc -l < gen/peg_yaml.sh) lines)"
  else
    echo '  ✗ gen/peg_yaml.sh not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-bash.lisp ]; then
    echo "  ✓ spec/yaml-peg-bash.lisp ($(wc -l < spec/yaml-peg-bash.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-bash.lisp not found'
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 bash gen/peg_yaml.sh "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 bash gen/peg_yaml.sh "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Bash: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
