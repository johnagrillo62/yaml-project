...
do_check() {
  ...
  if [ -f gen/peg_yaml.ml ]; then
    echo "  ✓ gen/peg_yaml.ml ($(wc -l < gen/peg_yaml.ml) lines)"
  else
    echo '  ✗ gen/peg_yaml.ml not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-ocaml.lisp ]; then
    echo "  ✓ spec/yaml-peg-ocaml.lisp ($(wc -l < spec/yaml-peg-ocaml.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-ocaml.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  ocamlopt -O2 -o bin/ocaml unix.cmxa gen/peg_yaml.ml
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 bin/ocaml "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 bin/ocaml "$sd/in.yaml" 2>/dev/null) || true
