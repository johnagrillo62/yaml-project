...
do_check() {
  ...
  if [ -f gen/peg_json.cpp ]; then
    echo "  ✓ gen/peg_json.cpp ($(wc -l < gen/peg_json.cpp) lines)"
  else
    echo '  ✗ gen/peg_json.cpp not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-cpp.lisp ]; then
    echo "  ✓ spec/json-peg-cpp.lisp ($(wc -l < spec/json-peg-cpp.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-cpp.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  g++ -std=c++17 -O2 -o bin/cpp gen/peg_json.cpp
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/cpp "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Cpp: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
