#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-ocaml.sh — Ocaml YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Ocaml YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v ocamlopt >/dev/null 2>&1; then
    echo "  ✓ ocamlopt $(ocamlopt --version 2>&1 | head -1)"
  else
    echo '  ✗ ocamlopt not found'
    echo '    Install: https://ocaml.org/install'
    fail=1
  fi
  if [ -f gen/peg_json.ml ]; then
    echo "  ✓ gen/peg_json.ml ($(wc -l < gen/peg_json.ml) lines)"
  else
    echo '  ✗ gen/peg_json.ml not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-ocaml.lisp ]; then
    echo "  ✓ spec/json-peg-ocaml.lisp ($(wc -l < spec/json-peg-ocaml.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-ocaml.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  ocamlopt -O2 -o bin/ocaml unix.cmxa gen/peg_json.ml
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/ocaml "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Ocaml: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
