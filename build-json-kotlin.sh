#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-kotlin.sh — Kotlin YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Kotlin YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v kotlinc >/dev/null 2>&1; then
    echo "  ✓ kotlinc $(kotlinc -version 2>&1 | grep -i kotlin | head -1)"
  else
  ...
  if command -v java >/dev/null 2>&1; then
    echo "  ✓ java $(kotlinc -version 2>&1 | grep -i kotlin | head -1)"
  else
    echo '  ✗ java not found'
    echo '    Install: apt install default-jre  OR  brew install openjdk'
    fail=1
  fi
  if [ -f gen/PegJson.kt ]; then
    echo "  ✓ gen/PegJson.kt ($(wc -l < gen/PegJson.kt) lines)"
  else
    echo '  ✗ gen/PegJson.kt not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-kotlin.lisp ]; then
    echo "  ✓ spec/json-peg-kotlin.lisp ($(wc -l < spec/json-peg-kotlin.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-kotlin.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  kotlinc gen/PegJson.kt -include-runtime -d bin/kotlin.jar 2>/dev/null
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 5 java -jar bin/kotlin.jar "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Kotlin: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
