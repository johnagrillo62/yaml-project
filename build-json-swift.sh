#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-swift.sh — Swift YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Swift YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v swiftc >/dev/null 2>&1; then
    ...
    echo '    Install: https://swift.org/download/'
    fail=1
  fi
  if [ -f gen/PegJson.swift ]; then
    echo "  ✓ gen/PegJson.swift ($(wc -l < gen/PegJson.swift) lines)"
  else
    echo '  ✗ gen/PegJson.swift not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-swift.lisp ]; then
    echo "  ✓ spec/json-peg-swift.lisp ($(wc -l < spec/json-peg-swift.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-swift.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  swiftc -O -o bin/swift gen/PegJson.swift
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/swift "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Swift: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
