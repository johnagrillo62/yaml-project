...
do_check() {
  ...
  if [ -f gen/PegYaml.cs ]; then
    echo "  ✓ gen/PegYaml.cs ($(wc -l < gen/PegYaml.cs) lines)"
  else
    echo '  ✗ gen/PegYaml.cs not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-csharp.lisp ]; then
    echo "  ✓ spec/yaml-peg-csharp.lisp ($(wc -l < spec/yaml-peg-csharp.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-csharp.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  mcs -out:bin/csharp.exe gen/PegYaml.cs
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 mono bin/csharp.exe "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 mono bin/csharp.exe "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Csharp: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
