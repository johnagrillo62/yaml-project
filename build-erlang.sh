#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-erlang.sh — Erlang YAML 1.2 parser demo
# 
...
# 
# Source: gen/peg_yaml.erl
# Spec:   spec/yaml-peg-erlang.lisp
# ════════════════════════════════════════════════════════════════
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Erlang YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v escript >/dev/null 2>&1; then
    echo "  ✓ escript $(erl +V 2>&1 | head -1)"
  else
    echo '  ✗ escript not found'
    echo '    Install: apt install erlang  OR  brew install erlang'
    fail=1
  fi
  if [ -f gen/peg_yaml.erl ]; then
    echo "  ✓ gen/peg_yaml.erl ($(wc -l < gen/peg_yaml.erl) lines)"
  else
    echo '  ✗ gen/peg_yaml.erl not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-erlang.lisp ]; then
    echo "  ✓ spec/yaml-peg-erlang.lisp ($(wc -l < spec/yaml-peg-erlang.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-erlang.lisp not found'
...
do_test() {
  echo '⚠  Erlang spawns the BEAM VM per test — this may take several minutes'
  echo 
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 escript gen/peg_yaml.erl "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 escript gen/peg_yaml.erl "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Erlang: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
