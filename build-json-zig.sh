...
do_check() {
  ...
  if [ -f gen/peg_json.zig ]; then
    echo "  ✓ gen/peg_json.zig ($(wc -l < gen/peg_json.zig) lines)"
  else
    echo '  ✗ gen/peg_json.zig not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-zig.lisp ]; then
    echo "  ✓ spec/json-peg-zig.lisp ($(wc -l < spec/json-peg-zig.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-zig.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  zig build-exe -O ReleaseFast -femit-bin=bin/zig gen/peg_json.zig
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/zig "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Zig: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
