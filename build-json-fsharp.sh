...
do_check() {
  ...
  if [ -f gen/PegJson.fs ]; then
    echo "  ✓ gen/PegJson.fs ($(wc -l < gen/PegJson.fs) lines)"
  else
    echo '  ✗ gen/PegJson.fs not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-fsharp.lisp ]; then
    echo "  ✓ spec/json-peg-fsharp.lisp ($(wc -l < spec/json-peg-fsharp.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-fsharp.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  mkdir -p bin/fsharp-json && [ -f bin/fsharp-json/fsharp-json.fsproj ] || dotnet new console -lang F# --force -o bin/fsharp-json >/dev/null 2>&1 && cp gen/PegJson.fs bin/fsharp-json/Program.fs && dotnet publish bin/fsharp-json -c Release -o bin/fsharp-json-out --self-contained >/dev/null 2>&1
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 3 bin/fsharp-json-out/fsharp-json "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Fsharp: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
