#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-haskell.sh — Haskell YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Haskell YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v ghc >/dev/null 2>&1; then
    echo "  ✓ ghc $(ghc --version 2>&1 | head -1)"
  else
    echo '  ✗ ghc not found'
    echo '    Install: https://www.haskell.org/ghcup/'
    fail=1
  fi
  if [ -f gen/PegJson.hs ]; then
    echo "  ✓ gen/PegJson.hs ($(wc -l < gen/PegJson.hs) lines)"
  else
    echo '  ✗ gen/PegJson.hs not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-haskell.lisp ]; then
    echo "  ✓ spec/json-peg-haskell.lisp ($(wc -l < spec/json-peg-haskell.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-haskell.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  ghc -O2 -o bin/haskell gen/PegJson.hs
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/haskell "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Haskell: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
