...
do_check() {
  ...
  if [ -f gen/peg_json.erl ]; then
    echo "  ✓ gen/peg_json.erl ($(wc -l < gen/peg_json.erl) lines)"
  else
    echo '  ✗ gen/peg_json.erl not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-erlang.lisp ]; then
    echo "  ✓ spec/json-peg-erlang.lisp ($(wc -l < spec/json-peg-erlang.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-erlang.lisp not found'
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 escript gen/peg_json.erl "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Erlang: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
