...
do_check() {
  ...
  if [ -f gen/PegYaml.hs ]; then
    echo "  ✓ gen/PegYaml.hs ($(wc -l < gen/PegYaml.hs) lines)"
  else
    echo '  ✗ gen/PegYaml.hs not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-haskell.lisp ]; then
    echo "  ✓ spec/yaml-peg-haskell.lisp ($(wc -l < spec/yaml-peg-haskell.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-haskell.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  ghc -O2 -o bin/haskell gen/PegYaml.hs
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  echo "════════════════════════════════════════════════════"
  echo " Haskell: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
