...
and c_b_block_header inp n =
    ...
    let r_ =
        pegAlt ([
                ...
                (fun inp -> detectIndent n inp)]) inp
    if r_.failed then r_
    else
    let m = r_.tagInt
    let inp = r_.rest
    let r_ =
...
and c_lliteral inp n =
    ...
    let t = r_.tag
    let inp = r_.rest
    pegSeq ([(fun inp -> s_b_comment inp); (fun inp -> l_literal_content inp (n + m) t)]) inp)]) inp
...
and c_lfolded inp n =
    ...
    let t = r_.tag
    let inp = r_.rest
    pegSeq ([(fun inp -> s_b_comment inp); (fun inp -> l_folded_content inp (n + m) t)]) inp)]) inp
...
and lblock_sequence inp n =
    ...
    let m = r_.tagInt
    let inp = r_.rest
    collectFn (fun inp -> plus_ (fun inp -> pegSeq ([(fun inp -> s_indent inp (n + m)); (fun inp -> c_l_block_seq_entry inp (n + m))]) inp) inp) inp) inp
...
and lblock_mapping inp n =
    ...
    let m = r_.tagInt
    let inp = r_.rest
    collectFn (fun inp -> plus_ (fun inp -> pegSeq ([(fun inp -> s_indent inp (n + m)); (fun inp -> ns_l_block_map_entry inp (n + m))]) inp) inp) inp) inp
