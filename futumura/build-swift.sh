...
do_check() {
  ...
  if [ -f gen/PegYaml.swift ]; then
    echo "  ✓ gen/PegYaml.swift ($(wc -l < gen/PegYaml.swift) lines)"
  else
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  swiftc -O -o bin/swift gen/PegYaml.swift
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 bin/swift "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 bin/swift "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Swift: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
}
