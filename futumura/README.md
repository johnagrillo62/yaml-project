...
One grammar. Twelve languages. Every parser passes **308/308** valid
tests from the [YAML Test
Suite](https://github.com/yaml/yaml-test-suite). Generated in seconds
...
Each target language needs a *target spec* — approximately 200 lines that tells the projector how the language expresses functions, closures, sequences, alternatives, and pattern matching. This is the only hand-written part, and AI can create it.

Given an existing target spec as a reference, AI can produce a new spec for any language in a single conversation. The projector then generates the complete parser — 1800-2200 lines — in seconds. The output is deterministic: the same grammar and target spec always produce the same parser.
