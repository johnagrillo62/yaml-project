...
impl YamlConverter {
    ...
    fn convert(&mut self, node: &Ast) -> YamlValue {
        match node {
            Ast::Leaf(text) => coerce_scalar(text),
            Ast::Branch(tag, children) => {
                match tag.as_str() {
                    \"ANCHOR\" => {
                        ...
                        for c in children {
                            if let Ast::Leaf(t) = c { if name.is_empty() { name = t.clone(); continue; } }
                            val = self.convert(c);
                        }
                    ...
                    \"ALIAS\" => {
                        for c in children {
                            if let Ast::Leaf(t) = c {
                                if let Some(v) = self.anchors.get(t) { return v.clone(); }
                            }
                        }
                        YamlValue::Null
                    }
                    \"MAPPING\" => {
                        ...
                        for c in children {
                            if let Ast::Branch(ref ct, ref cc) = c {
                                if ct == \"PAIR\" && cc.len() >= 2 {
                                    let key = self.convert(&cc[0]);
                                    let val = self.convert(&cc[1]);
                                    if let (YamlValue::Str(ref k), YamlValue::Map(ref merge)) = (&key, &val) {
                                        if k == \"<<\" {
                                            for (mk, mv) in merge { m.entry(mk.clone()).or_insert_with(|| mv.clone()); }
                                            continue;
                                        }
                                    }
                    ...
                    \"SEQUENCE\" => YamlValue::Seq(children.iter().map(|c| self.convert(c)).collect()),
                    \"DOC\" | \"STREAM\" => {
                        if children.len() == 1 { return self.convert(&children[0]); }
                        let docs: Vec<_> = children.iter().map(|c| self.convert(c)).collect();
                        if docs.len() == 1 { docs.into_iter().next().unwrap() } else { YamlValue::Seq(docs) }
                    }
                    _ => {
                        if children.len() == 1 { return self.convert(&children[0]); }
                        YamlValue::Seq(children.iter().map(|c| self.convert(c)).collect())
                    }
                }
            }
        }
...
fn yaml_load(text: &str) -> YamlValue {
    let s = text.to_string();
    let inp = Input { src: &s as *const String, pos: 0, line: 1, col: 0 };
    let r = l_yaml_stream(inp);
    if r.fail { return YamlValue::Null; }
    match r.ast {
        Some(ast) => YamlConverter::new().convert(&ast),
        None => YamlValue::Null,
    }
}")
