...
do_check() {
  ...
  if [ -f gen/peg_json.rs ]; then
    echo "  ✓ gen/peg_json.rs ($(wc -l < gen/peg_json.rs) lines)"
  else
    echo '  ✗ gen/peg_json.rs not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-rust.lisp ]; then
    echo "  ✓ spec/json-peg-rust.lisp ($(wc -l < spec/json-peg-rust.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-rust.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  rustc -O -o bin/rust gen/peg_json.rs
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/rust "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Rust: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
