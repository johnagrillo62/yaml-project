...
do_check() {
  ...
  if [ -f gen/PegYaml.kt ]; then
    echo "  ✓ gen/PegYaml.kt ($(wc -l < gen/PegYaml.kt) lines)"
  else
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  kotlinc gen/PegYaml.kt -include-runtime -d bin/kotlin.jar 2>/dev/null
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 5 java -jar bin/kotlin.jar "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 5 java -jar bin/kotlin.jar "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Kotlin: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
