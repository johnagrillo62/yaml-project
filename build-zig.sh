#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-zig.sh — Zig YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Zig YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v zig >/dev/null 2>&1; then
    echo "  ✓ zig $(zig --version 2>&1 | head -1)"
  else
    echo '  ✗ zig not found'
    echo '    Install: https://ziglang.org/download/'
    fail=1
  fi
  if [ -f gen/peg_yaml.zig ]; then
    echo "  ✓ gen/peg_yaml.zig ($(wc -l < gen/peg_yaml.zig) lines)"
  else
    echo '  ✗ gen/peg_yaml.zig not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-zig.lisp ]; then
    echo "  ✓ spec/yaml-peg-zig.lisp ($(wc -l < spec/yaml-peg-zig.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-zig.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  zig build-exe -O ReleaseFast -femit-bin=bin/zig gen/peg_yaml.zig
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 bin/zig "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 bin/zig "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Zig: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
