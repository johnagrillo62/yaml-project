#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-objc.sh — Objc YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Objc YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if [ -f gen/PegJson.m ]; then
    echo "  ✓ gen/PegJson.m ($(wc -l < gen/PegJson.m) lines)"
  else
    echo '  ✗ gen/PegJson.m not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-objc.lisp ]; then
    echo "  ✓ spec/json-peg-objc.lisp ($(wc -l < spec/json-peg-objc.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-objc.lisp not found'
...
do_build() {
  ...
  if [ "$(uname)" = "Darwin" ]; then
    clang -framework Foundation -O2 -o bin/objc gen/PegJson.m
  else
    clang $(gnustep-config --objc-flags) -I/usr/lib/gcc/x86_64-linux-gnu/13/include -fblocks -o bin/objc gen/PegJson.m $(gnustep-config --base-libs) -lBlocksRuntime
  fi
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 bin/objc "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Objc: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
