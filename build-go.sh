...
do_check() {
  ...
  if [ -f gen/peg_yaml.go ]; then
    echo "  ✓ gen/peg_yaml.go ($(wc -l < gen/peg_yaml.go) lines)"
  else
    echo '  ✗ gen/peg_yaml.go not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/yaml-peg-go.lisp ]; then
    echo "  ✓ spec/yaml-peg-go.lisp ($(wc -l < spec/yaml-peg-go.lisp) lines)"
  else
    echo '  ✗ spec/yaml-peg-go.lisp not found'
...
do_build() {
  echo 'Building...'
  mkdir -p bin
  go build -o bin/go gen/peg_yaml.go
  echo '  ✓ Build complete'
  echo 
}
...
do_test() {
  ...
  for d in yaml-test-suite/*/; do
    ...
    for sd in $dirs; do
      ...
      if [ -f "$sd/error" ]; then
        rc=0; (timeout 1 bin/go "$sd/in.yaml" >/dev/null 2>&1) || rc=$?
        [ $rc -ne 0 ] && pass=$((pass+1)) || fail=$((fail+1))
      else
        result=$(timeout 1 bin/go "$sd/in.yaml" 2>/dev/null) || true
  ...
  echo "════════════════════════════════════════════════════"
  echo " Go: $pass / $total passed  ($fail failed)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
