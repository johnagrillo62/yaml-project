#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
# build-lua.sh — Lua YAML 1.2 parser demo
# 
...
show_system() {
  echo "════════════════════════════════════════════════════"
  echo " Lua YAML Parser — Futumura Projector Demo"
  echo "════════════════════════════════════════════════════"
  echo 
...
do_check() {
  ...
  if command -v lua >/dev/null 2>&1; then
    ...
    echo '  ✗ lua not found'
    echo '    Install: apt install lua5.4  OR  brew install lua'
    fail=1
  fi
  if [ -f gen/peg_json.lua ]; then
    echo "  ✓ gen/peg_json.lua ($(wc -l < gen/peg_json.lua) lines)"
  else
    echo '  ✗ gen/peg_json.lua not found'
    echo '    Run: sbcl --load build-yaml.lisp --quit'
    fail=1
  fi
  if [ -f spec/json-peg-lua.lisp ]; then
    echo "  ✓ spec/json-peg-lua.lisp ($(wc -l < spec/json-peg-lua.lisp) lines)"
  else
    echo '  ✗ spec/json-peg-lua.lisp not found'
...
do_test() {
  ...
  for f in JSONTestSuite/test_parsing/y_*.json; do
    ...
    rc=0; (timeout 1 lua gen/peg_json.lua "$f" >/dev/null 2>&1) || rc=$?
  ...
  echo "════════════════════════════════════════════════════"
  echo " Lua: $pass / $total passed  ($fail failed, $skip impl-defined)  ${elapsed}${unit}"
  echo "════════════════════════════════════════════════════"
  echo 
