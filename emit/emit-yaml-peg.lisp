...
(defun x86-resolve-arg (arg reg stream)
  "For x86: resolve ARG into REG, writing to STREAM.
   If ARG is an ARITH: marker, call the wrapper and load from _arith_tmp.
   Otherwise, emit mov REG, ARG directly."
  (if (and (stringp arg) (>= (length arg) 6) (string= (subseq arg 0 6) "ARITH:"))
      (let ((wrapper (subseq arg 6)))
        (format stream "call ~A~%    mov ~A, qword [_arith_tmp]~%    " wrapper reg))
      (format stream "mov ~A, ~A~%    " reg arg)))
    ...
    (cond
      ...
      ((and style (string= style "powershell"))
       (if args
           ...
           (format nil "~A $~A" fn iv)))
      ((and style (string= style "x86"))
       (if args
           (let ((regs '("rdi" "rsi" "rdx" "rcx" "r8" "r9")))
             (with-output-to-string (s)
               (loop for a in args
                     for r in regs
                     do (x86-resolve-arg a r s))
               (format s "call ~A" fn)))
           (format nil "call ~A" fn)))
      (t
       (if args
           (format nil "~A(~A, ~{~A~^, ~})" fn iv args)
           (format nil "~A(~A)" fn iv))))))
             ...
             (loop for a in args
                   ...
                   do (if (and (stringp a) (>= (length a) 6) (string= (subseq a 0 6) "ARITH:"))
                          (let ((wrapper (subseq a 6)))
                            (format s "call ~A~%    mov rax, qword [_arith_tmp]~%    mov [~A], rax~%    " wrapper p))
                          (format s "mov rax, ~A~%    mov [~A], rax~%    " a p)))
             (format s "call ~A" fn))
           (format nil "call ~A" fn)))
      (t
       (if args
...
(defun bash-style-p ()
  "True for call styles that use wrapper functions (bash, wrapper, and x86)."
  (let ((s (tgt "call-style")))
    (and s (or (string= s "bash") (string= s "wrapper") (string= s "x86")))))
...
(defun ml-join (wrapped)
  ...
  (let* ((pad (indent-str))
         (comma (cond ((and (tgt "call-style") (string= (tgt "call-style") "bash")) "")
                      ((and (tgt "call-style") (string= (tgt "call-style") "x86")) "")
  ...
  (if (= 1 (length exprs)) (ce (car exprs) env)
      ...
      (let ((wrapped (mapcar (lambda (e) (bw (ce e env) env)) exprs)))
        (let ((one-line (funcall (tgt "seq-emit") wrapped)))
          (if (or (bash-style-p) (<= (length one-line) *max-line-length*))
              one-line
  ...
  (cond
    ...
    ((and (listp expr) (eq (car expr) '+))
     (let ((parts (mapcar (lambda (e) (ca e env)) (cdr expr))))
       (if (pure-bash-p)
           (format nil "$(( ~{~A~^ + ~} ))" parts)
           (if (and (tgt "call-style") (string= (tgt "call-style") "x86"))
               ;; x86: compute in wrapper, return reference to _arith_tmp
               ;; The wrapper computes and stores; we emit call+ref as two lines
               ;; that rule-call-with-params / trcall handle specially
               (let ((w (make-bash-wrapper
                         (with-output-to-string (s)
                           (format s "mov rax, ~A~%" (car parts))
                           (dolist (p (cdr parts))
                             (format s "    add rax, ~A~%" p))
                           (format s "    mov [_arith_tmp], rax")))))
                 ;; Return a marker that the x86 callers understand
               ...
               (let ((w (make-bash-wrapper
                         (with-output-to-string (s)
                           (if (= 1 (length ps))
                               (progn
                                 (format s "mov rax, 0~%")
                                 (format s "    sub rax, ~A~%" (car ps)))
                               (progn
                                 (format s "mov rax, ~A~%" (car ps))
                                 (dolist (p (cdr ps))
                                   (format s "    sub rax, ~A~%" p))))
                           (format s "    mov [_arith_tmp], rax")))))
                 (format nil "ARITH:~A" w))
...
(defun emit-peg (gram path)
  (let ((*gram* gram)
    ...
    ;; Reset target-specific accumulators
    (when (boundp '*x86-strings*) (setq *x86-strings* nil))
    (when (boundp '*x86-string-counter*) (setq *x86-string-counter* 0))
    (with-open-file (*out* path :direction :output :if-exists :supersede :external-format :utf-8)
      ...
      (let ((cmt (or (tgt "comment-prefix") "//"))
        ...
        ;; Pre-wrappers hook (e.g., x86 string data)
        (let ((pre-fn (tgt "pre-wrappers")))
